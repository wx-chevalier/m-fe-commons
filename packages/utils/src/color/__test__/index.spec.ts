...
describe('TinyColor', () => {
  ...
  it('should parse options', () => {
    expect(new TinyColor('red', { format: 'hex' }).toString()).toEqual(
      '#ff0000',
    );
    expect(
      fromRatio({ r: 1, g: 0, b: 0 }, { format: 'hex' }).toString(),
    ).toEqual('#ff0000');
  });
  ...
  it('should parse ratio', () => {
    ...
    expect(fromRatio({ r: 1, g: 0, b: 0, a: 0.5 }).toRgbString()).toBe(
      'rgba(255, 0, 0, 0.5)',
    );
    // alpha = 1 works when ratio is parsed
    expect(fromRatio({ r: 1, g: 0, b: 0, a: 1 }).toRgbString()).toBe(
      'rgb(255, 0, 0)',
    );
    // alpha > 1 works when ratio is parsed
    expect(fromRatio({ r: 1, g: 0, b: 0, a: 10 }).toRgbString()).toBe(
      'rgb(255, 0, 0)',
    );
    // alpha < 1 works when ratio is parsed
    expect(fromRatio({ r: 1, g: 0, b: 0, a: -1 }).toRgbString()).toBe(
    ...
    expect(new TinyColor({ r: 0.1, g: 0.1, b: 0.1 }).toHexString()).toBe(
      '#000000',
    );
  ...
  it('should parse rgb', () => {
    ...
    expect(
      new TinyColor({ r: 200, g: 100, b: 0 }).equals('rgb(200, 100, 0)'),
    ).toBe(true);
    expect(
      new TinyColor({ r: 200, g: 100, b: 0 }).equals('rgb 200 100 0'),
    ).toBe(true);
    ...
    expect(
      new TinyColor().equals(new TinyColor({ r: 200, g: 100, b: 0 })),
    ).toBe(false);
    expect(
      new TinyColor().equals(new TinyColor({ r: 200, g: 100, b: 0 })),
    ).toBe(false);
  ...
  it('should parse percentage rgb text', () => {
    ...
    expect(new TinyColor('rgb (100%, 0%, 0%)').toHexString()).toBe('#ff0000');
    // object input
    expect(new TinyColor({ r: '100%', g: '0%', b: '0%' }).toHexString()).toBe(
      '#ff0000',
    );
    // object input and compare
    ...
    expect(
      new TinyColor({ r: '90%', g: '45%', b: '0%' }).equals(
        'rgb(90%, 45%, 0%)',
      ),
    ).toBe(true);
    expect(
      new TinyColor({ r: '90%', g: '45%', b: '0%' }).equals('rgb 90% 45% 0%'),
    ).toBe(true);
    expect(
      new TinyColor({ r: '90%', g: '45%', b: '0%' }).equals('rgb 90% 45% 0%'),
    ).toBe(true);
    expect(
      new TinyColor({ r: '90%', g: '45%', b: '0%', a: 0.4 }).equals(
        'rgba 90% 45% 0% .4',
      ),
    ).toBe(true);
    expect(
      new TinyColor({ r: '89%', g: '45%', b: '0%' }).equals(
        'rgba 90% 45% 0% 1',
      ),
    ).toBe(false);
    ...
    expect(
      new TinyColor({ r: '89%', g: '45%', b: '0%' }).equals('rgb 90% 45% 0%'),
    ).toBe(false);
    expect(
      new TinyColor({ r: '89%', g: '45%', b: '0%' }).equals('rgb 90% 45% 0%'),
    ).toBe(false);
  ...
  it('should parse hsv string', () => {
    ...
    expect(new TinyColor('hsv 251.1 0.887 .918').toHsvString()).toBe(
      'hsv(251, 89%, 92%)',
    );
    expect(new TinyColor('hsv 251.1 0.887 0.918').toHsvString()).toBe(
      'hsv(251, 89%, 92%)',
    );
  ...
  it('should translate toString with alpha set', () => {
    ...
    const transparentNamed = fromRatio(
      { r: 255, g: 0, b: 0, a: 0 },
      { format: 'name' },
    );
  ...
  it('Alpha = 0 should act differently on toName()', () => {
    expect(new TinyColor({ r: 255, g: 20, b: 10, a: 0 }).toName()).toBe(
      'transparent',
    );
  ...
  it('isReadable', () => {
    ...
    expect(
      isReadable('#000000', '#ffffff', { level: 'AA', size: 'small' }),
    ).toBe(true);
    expect(isReadable('#ff0088', '#5c1a72', {})).toBe(false);
    expect(
      isReadable('#ff0088', '#8822aa', { level: 'AA', size: 'small' }),
    ).toBe(false);
    expect(
      isReadable('#ff0088', '#8822aa', { level: 'AA', size: 'large' }),
    ).toBe(false);
    expect(
      isReadable('#ff0088', '#8822aa', { level: 'AAA', size: 'small' }),
    ).toBe(false);
    expect(
      isReadable('#ff0088', '#8822aa', { level: 'AAA', size: 'large' }),
    ).toBe(false);
    ...
    expect(
      isReadable('#ff0088', '#5c1a72', { level: 'AAA', size: 'small' }),
    ).toBe(false);
    expect(
      isReadable('#ff0088', '#5c1a72', { level: 'AAA', size: 'large' }),
    ).toBe(false);

    // "#ff0088", "#2e0c3a": contrast ratio 4.56
    expect(
      isReadable('#ff0088', '#2e0c3a', { level: 'AA', size: 'small' }),
    ).toBe(true);
    expect(
      isReadable('#ff0088', '#2e0c3a', { level: 'AA', size: 'large' }),
    ).toBe(true);
    expect(
      isReadable('#ff0088', '#2e0c3a', { level: 'AAA', size: 'small' }),
    ).toBe(false);
    expect(
      isReadable('#ff0088', '#2e0c3a', { level: 'AAA', size: 'large' }),
    ).toBe(true);

    // "#db91b8", "#2e0c3a":  contrast ratio 7.12
    expect(
      isReadable('#db91b8', '#2e0c3a', { level: 'AA', size: 'small' }),
    ).toBe(true);
    expect(
      isReadable('#db91b8', '#2e0c3a', { level: 'AA', size: 'large' }),
    ).toBe(true);
    expect(
      isReadable('#db91b8', '#2e0c3a', { level: 'AAA', size: 'small' }),
    ).toBe(true);
    expect(
      isReadable('#db91b8', '#2e0c3a', { level: 'AAA', size: 'large' }),
    ).toBe(true);
    expect(
      isReadable('#db91b8', '#2e0c3a', { level: 'ZZZ', size: 'large' } as any),
    ).toBe(false);
  });
  it('readability', () => {
  ...
  it('mostReadable', () => {
    expect(mostReadable('#000', ['#111', '#222'])!.toHexString()).toBe(
      '#222222',
    );
    expect(mostReadable('#f00', ['#d00', '#0d0'])!.toHexString()).toBe(
      '#00dd00',
    );
    expect(
    ...
    expect(mostReadable('#fff', ['#fff', '#fff'])!.toHexString()).toBe(
      '#ffffff',
    );
  ...
  it('Spin', () => {
    ...
    [-360, 0, 360].forEach(delta => {
      Object.keys(names).forEach(name => {
        expect(new TinyColor(name).toHex()).toBe(
          new TinyColor(name).spin(delta).toHex(),
        );
      });
    });
  });
  it('Mix', () => {
    ...
    for (let i = 0; i < 100; i++) {
      expect(
        Math.round(new TinyColor('#000').mix('#fff', i).toHsl().l * 100) / 100,
      ).toBe(i / 100);
    }
    ...
    for (let i = 0; i < 100; i++) {
      ...
      expect(new TinyColor('#f00').mix('#000', i).toHex()).toBe(
        newHex + '0000',
      );
      expect(new TinyColor('#0f0').mix('#000', i).toHex()).toBe(
        `00${newHex}00`,
      );
      expect(new TinyColor('#00f').mix('#000', i).toHex()).toBe(
        '0000' + newHex,
      );
      expect(new TinyColor('transparent').mix('#000', i).toRgb().a).toBe(
        i / 100,
      );
    }
  });
  ...
  it('analogous', () => {
    ...
    expect(colorsToHexString(combination)).toBe(
  ...
  it('should handle conversion to number', () => {
    expect(tinycolor('rgb(18, 0, 0)').toString()).toBe(
      tinycolor(0x120000).toString(),
    );
    expect(tinycolor('rgb(0, 52, 0)').toString()).toBe(
      tinycolor(0x3400).toString(),
    );
    expect(tinycolor('rgb(0, 0, 86)').toString()).toBe(
      tinycolor(0x56).toString(),
    );
