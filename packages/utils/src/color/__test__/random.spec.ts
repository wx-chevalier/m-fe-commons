...
describe('random', () => {
  ...
  it('should accept hue', () => {
    let colors = random({ hue: 'purple', count: 3, seed: 11100 }).map(n =>
      n.toHexString(),
    );
    expect(colors).toEqual(['#9b22e6', '#9f1ceb', '#a316f0']);
    colors = random({ hue: 'red', count: 3, seed: 13371337 }).map(n =>
      n.toHexString(),
    );
    expect(colors).toEqual(['#e34236', '#e34230', '#e8432a']);
    colors = random({ hue: 'blue', count: 3, seed: 13371337 }).map(n =>
      n.toHexString(),
    );
    expect(colors).toEqual(['#3346d6', '#2e39d9', '#2828de']);
    colors = random({ hue: 'purple', count: 3, seed: 13371337 }).map(n =>
      n.toHexString(),
    );
    expect(colors).toEqual(['#9335db', '#952fde', '#9929e3']);
    colors = random({ hue: 'orange', count: 3, seed: 13371337 }).map(n =>
      n.toHexString(),
    );
    expect(colors).toEqual(['#e8a438', '#ebaa31', '#edac2b']);
    colors = random({ hue: 'pink', count: 3, seed: 13371337 }).map(n =>
      n.toHexString(),
    );
    expect(colors).toEqual(['#f03ab9', '#f032b1', '#f22ca9']);
    colors = random({ hue: '#E6E6FA', count: 3, seed: 420420 }).map(n =>
      n.toHexString(),
    );
    expect(colors).toEqual(['#4141d1', '#3939d4', '#3333d6']);
    colors = random({ hue: 999, count: 3, seed: 420420 }).map(n =>
      n.toHexString(),
    );
    expect(colors).toEqual(['#4167d1', '#393cd4', '#5733d6']);
    colors = random({ hue: 'monochrome', count: 3, seed: 420420 }).map(n =>
      n.toHexString(),
    );
    expect(colors).toEqual(['#9e9e9e', '#a8a8a8', '#b3b3b3']);
    colors = random({ hue: NaN, count: 3, seed: 420420 }).map(n =>
      n.toHexString(),
    );
    expect(colors).toEqual(['#4167d1', '#393cd4', '#5733d6']);
  });
  it('should accept luminosity', () => {
    ...
    colors = random({ luminosity: 'dark', count: 3, seed: 9999923 }).map(n =>
      n.toHexString(),
    );
  ...
  it('should accept luminosity', () => {
    ...
    expect(colors).toEqual(['#06377a', '#05197d', '#0d0580']);
    colors = random({ luminosity: 'light', count: 3, seed: 9999923 }).map(n =>
      n.toHexString(),
    );
