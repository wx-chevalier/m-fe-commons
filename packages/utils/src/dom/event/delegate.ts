...
const elements = new WeakMap<
  EventTarget,
  WeakMap<DelegateEventHandler<any, any>, Set<Setup>>
>();
...
>(
  ...
  const elementMap =
    ...
    new WeakMap<DelegateEventHandler<TEvent, TElement>, Set<Setup>>();
  ...
  elements.set(
    element,
    elementMap.set(callback, setups.add({ selector, type, capture })),
  );
...
export function delegate<
  TElement extends Element = Element,
  TEvent extends Event = Event
>(elements: any, selector: any, type: any, callback?: any, options?: any): any {
  // Handle the regular Element usage
  if (typeof (elements as EventTarget).addEventListener === 'function') {
    return _delegate<TElement, TEvent>(
