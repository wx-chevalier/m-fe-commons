...
export default class Mark {
   ...
   *
   */
  constructor(el: HTMLElement, options: MarkOptions = {}) {
    if (el && el.nodeType === 1) {
      this.el = el;
    } else {
...
const genImgDataURL = memoize((text: string, opacity: number) => {
...
function memoize(func: any) {
  ...
  return function _memoize(...args: any[]) {
