...
import { NoticeIconData } from '../../types/model';
...
export interface NoticeIconProps {
  ...
  NoticeList: React.ComponentType<NoticeIconTabProps>;
...
export class NoticeIcon extends Component<NoticeIconProps> {
  ...
  getNotificationBox(): React.ReactNode {
    const { children, loading, clearText, viewMoreText, NoticeList } = this.props;
    ...
    return (
      <>
        <Spin spinning={loading} delay={300}>
          <Tabs className="m-fe-react-commons-notice-icon-tabs" onChange={this.onTabChange}>
            {panes}
          </Tabs>
        </Spin>
      </>
  ...
  render(): React.ReactNode {
    ...
    const noticeButtonClass = classNames(className, 'm-fe-react-commons-notice-icon-noticeButton');
    const notificationBox = this.getNotificationBox();
    const NoticeBellIcon = bell || (
      <BellOutlined className={'m-fe-react-commons-notice-icon-icon'} />
    );
    const trigger = (
      <span className={classNames(noticeButtonClass, { opened: visible })}>
        <Badge
          ...
          className={'m-fe-react-commons-notice-icon-badge'}
        >
          {NoticeBellIcon}
        </Badge>
      </span>
    ...
    return (
      <HeaderDropdown
        ...
        overlayClassName={'m-fe-react-commons-notice-icon-popover'}
        trigger={['click']}
        visible={visible}
